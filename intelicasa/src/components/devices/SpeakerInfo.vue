<template>
    <v-row align="center" justify="center">
        <v-icon v-if="device.volume === 0" icon="mdi-volume-low" class="mr-1"></v-icon>
        <v-icon v-else-if="device.volume < 5" icon="mdi-volume-medium" class="mr-1"></v-icon>
        <v-icon v-else icon="mdi-volume-high" class="mr-1"></v-icon>

        <v-btn density="compact" icon="mdi-minus" @click="device.volume--" />
        <v-sheet class="sliderCont">
            <v-slider v-model="device.volume" hide-details min="0" max="10" step="1"></v-slider>
        </v-sheet>
    ...
    <v-row align="center" display="flex" justify="center">
        <v-sheet class="sliderCont">
            <v-select class="centeredSelect" variant="outlined" :items=" device.genres " v-model=" device.genre "
                hide-details hide-selected label="Género" item-text="name" item-value="name" />
        </v-sheet>
    </v-row>
</template>
