<template>
    <DevicePower v-show="device.category.name !== 'Puerta' && device.category.name !== 'Aspiradora'" :device="device"
    ...
    <SpeakerInfo :disabled="disabled" v-else-if="device.category.name === 'Parlante'" :device="device"
        :returnAction="returnAction" @actionSet="(action) => emitAction(action)" />
    <VacuumDeviceInfo v-else-if="device.category.name === 'Aspiradora'" :device="device" :returnAction="returnAction"
